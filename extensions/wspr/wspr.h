...
//#define WSPR_APRINTF
#ifdef WSPR_APRINTF
#define wspr_aprintf(fmt, ...) printf("WSPR-%02d: " fmt, rx_chan, ##__VA_ARGS__)
#else
...
//#define WSPR_PRINTF
#ifdef WSPR_PRINTF
#define wspr_printf(fmt, ...) printf("WSPR-%02d: " fmt, rx_chan, ##__VA_ARGS__)

#if 1
#define wspr_pprintf(fmt, ...) printf("WSPR-%02d: " fmt, rx_chan, ##__VA_ARGS__)
#define wspr_pxprintf(fmt, ...) printf(fmt, ##__VA_ARGS__)
#else
#define wspr_pprintf(fmt, ...)
#define wspr_pxprintf(fmt, ...)
#endif

#define wspr_gprintf(fmt, ...) printf(fmt, ##__VA_ARGS__)
...
#define SYMTIME \
  (FSPS / FSRATE)  // symbol time: 256 samps @ 375 srate, 683 ms, 1.46 Hz
...
#define FMAX 110
//#define	FMIN		-150				// frequency range to
//search #define	FMAX		150
...
#define LEN_CALL \
  (12 + SPACE_FOR_NULL)  // max of AANLL, ppp/AANLLL, AANLL/ss, plus 2
...
void unpack50(u1_t *dat, u4_t *call_28b, u4_t *grid_pwr_22b, u4_t *grid_15b,
              u4_t *pwr_7b);
...
int unpk_(u1_t *decdata, char *call_loc_pow, char *callsign, char *grid,
          int *dBm);
