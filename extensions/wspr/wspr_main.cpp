...
static void wspr_status(wspr_t *w, int status, int resume) {
  ...
  // Send on each status change to eliminate effects of browser host clock
  // drift.
  struct timespec ts;
  clock_gettime(CLOCK_REALTIME, &ts);
  u4_t msec = ts.tv_nsec / 1000000;
  ext_send_msg(w->rx_chan, WSPR_DEBUG_MSG,
               "EXT WSPR_STATUS=%d WSPR_TIME_MSEC=%d=%d", status, ts.tv_sec,
               msec);
...
// compute FFTs incrementally during data capture rather than all at once at the
...
void WSPR_FFT(void *param) {
  ...
  while (1) {
    ...
    WSPR_CPX_t *id = wb->i_data[w->fft_ping_pong],
               *qd = wb->q_data[w->fft_ping_pong];

    // float maxiq = 1e-66, maxpwr = 1e-66;
    // int maxi=0;
    memset(wb->savg, 0, sizeof(wb->savg));

    // FIXME: A large noise burst can washout the w->pwr_sampavg which prevents
    // a proper peak list being created in the decoder. An individual signal can
    // be decoded fine in the presence of the burst. But if there is no peak in
    // the list the decoding process is never started! We've seen this problem
    // fairly frequently.
    ...
    renormalize(w, wb->savg, wb->smspec[WSPR_RENORM_FFT],
                wb->tmpsort[WSPR_RENORM_FFT]);

    for (j = 0; j < nbins_411; j++) {
      wb->smspec[WSPR_RENORM_FFT][j] = dB =
          10 * log10(wb->smspec[WSPR_RENORM_FFT][j]) - w->snr_scaling_factor;
    ...
    if (ext_send_msg_data(w->rx_chan, WSPR_DEBUG_MSG, WSPR_DATA, wb->ws,
                          nbins_411 + 1) < 0) {
      w->send_error = true;
    }
    ...
    wspr_printf("\n");
    w->not_launched = 0;
    TaskWakeup(w->WSPR_DecodeTask_id, TWF_CHECK_WAKING,
               TO_VOID_PARAM(w->rx_chan));
  }
}
...
void wspr_send_peaks(wspr_t *w, int start, int stop) {
  ...
  *s = '\0';
  ...
  for (j = start; j < stop; j++) w->pk_save[j] = w->pk_freq[j];

  // complete peak list sent on each update
  for (j = 0; j < w->npk; j++) {
    int bin_flags = w->pk_save[j].bin0 | w->pk_save[j].flags;
    n = sprintf(s, "%d:%s:", bin_flags, w->pk_save[j].snr_call);
    s += n;
  }
  ext_send_msg_encoded(w->rx_chan, WSPR_DEBUG_MSG, "EXT", "WSPR_PEAKS", "%s",
                       peaks_s);
}
...
void wspr_send_decode(wspr_t *w, int seq) {
  ...
  if (log_decodes && (d->r_valid == 1 || d->r_valid == 2 || d->r_valid == 3)) {
    ...
    if ((arct & 7) == 0) {
      ...
      //                   0920 -26  0.4  7.040121  0  AE7YQ  DM41 10770   37
      //                   (5.0 W)
      rcfprintf(
          w->rx_chan, printf_type,
          "WSPR DECODE:  UTC  dB   dT      Freq dF  Call   Grid    km  dBm\n");
    }
    arct++;
  }
  ...
  // TYPE3	ppp/c6cccc g6gggg kkkkk ppp (s)	; worst case, just let the fields
  // float

  if (d->r_valid == 1) {  // TYPE1
    ext_send_msg_encoded(
        w->rx_chan, WSPR_DEBUG_MSG, "EXT", "WSPR_DECODED",
        "%02d%02d %3.0f %4.1f %9.6f %2d  "
        "<a href='https://www.qrz.com/lookup/%s' target='_blank'>%-6s</a> "
        "<a href='http://www.levinecentral.com/ham/grid_square.php?Grid=%s' "
        "target='_blank'>%s</a> "
        "%5d  %3d (%s)",
        d->hour, d->min, d->snr, d->dt_print, d->freq_print, (int)d->drift1,
        d->call, d->call, d->grid, d->grid, (int)grid_to_distance_km(d->grid),
        d->dBm, W_s);
    if (log_decodes)
      rcfprintf(w->rx_chan, printf_type,
                "WSPR DECODE: %02d%02d %3.0f %4.1f %9.6f %2d  %-6s %s %5d  %3d "
                "(%s)\n",
                d->hour, d->min, d->snr, d->dt_print, d->freq_print,
                (int)d->drift1, d->call, d->grid,
    ...
    if (strcmp(d->call, "...") == 0) {
      ...
      if (log_decodes)
        rcfprintf(w->rx_chan, printf_type,
                  "WSPR DECODE: %02d%02d %3.0f %4.1f %9.6f %2d  ...            "
                  "    %3d (%s)\n",
                  d->hour, d->min, d->snr, d->dt_print, d->freq_print,
                  (int)d->drift1, d->dBm, W_s);
    } else {
      ext_send_msg_encoded(
          w->rx_chan, WSPR_DEBUG_MSG, "EXT", "WSPR_DECODED",
          "%02d%02d %3.0f %4.1f %9.6f %2d  "
      ...
      if (log_decodes)
        rcfprintf(
            w->rx_chan, printf_type,
            "WSPR DECODE: %02d%02d %3.0f %4.1f %9.6f %2d  %-17s  %3d (%s)\n",
            // kkkkk--ppp
            d->hour, d->min, d->snr, d->dt_print, d->freq_print, (int)d->drift1,
            d->call, d->dBm, W_s);
    }
    ...
    if (strcmp(d->call, "...") == 0) {
      ...
      if (log_decodes)
        rcfprintf(w->rx_chan, printf_type,
                  "WSPR DECODE: %02d%02d %3.0f %4.1f %9.6f %2d  ...  %6s %5d  "
                  "%3d (%s)\n",
                  d->hour, d->min, d->snr, d->dt_print, d->freq_print,
                  (int)d->drift1, d->grid, (int)grid_to_distance_km(d->grid),
                  d->dBm, W_s);
    } else {
      ext_send_msg_encoded(
          w->rx_chan, WSPR_DEBUG_MSG, "EXT", "WSPR_DECODED",
      ...
      if (log_decodes)
        rcfprintf(
            w->rx_chan, printf_type,
            "WSPR DECODE: %02d%02d %3.0f %4.1f %9.6f %2d  %s %s %d %d (%s)\n",
            d->hour, d->min, d->snr, d->dt_print, d->freq_print, (int)d->drift1,
...
void WSPR_Deco(void *param) {
  ...
  while (1) {
    ...
    if (start)
      wspr_printf("WSPR_DecodeTask TOTAL %.1f sec\n",
    ...
    shmem_ipc_invoke(SIG_IPC_WSPR + w->rx_chan, w->rx_chan,
                     NO_WAIT);  // invoke wspr_decode()

    int done;
    do {
      done =
          shmem_ipc_poll(SIG_IPC_WSPR + w->rx_chan, POLL_MSEC(250), w->rx_chan);
      // Use "if" instead of "while" here and a relatively slow polling interval
      // so that peak marker changes have some persistence.
      if (w->send_peaks_seq_parent <
          w->send_peaks_seq) {  // make sure each one is processed
        wspr_send_peaks(w, wb->send_peaks_q[w->send_peaks_seq_parent].start,
                        wb->send_peaks_q[w->send_peaks_seq_parent].stop);
        w->send_peaks_seq_parent++;
      }
      while (w->send_decode_seq_parent <
             w->send_decode_seq) {  // make sure each one is processed
    ...
    if (w->abort_decode) wspr_aprintf("DECO-P decoder aborted\n");

    // upload spots at the end of the decoding when there is less load on
    // wsprnet.org
    ...
    for (int i = 0; i < w->uniques; i++) {
      ...
      if (w->autorun) {
        int year, month, day;
        utc_year_month_day(&year, &month, &day);
        char *cmd;
        asprintf(&cmd,
                 "curl "
                 "'http://wsprnet.org/"
                 "post?function=wspr&rcall=%s&rgrid=%s&rqrg=%.6f&date=%02d%02d%"
                 "02d&time=%02d%02d&sig=%.0f&dt=%.1f&drift=%d&tqrg=%.6f&tcall=%"
                 "s&tgrid=%s&dbm=%s&version=1.3+Kiwi' >/dev/null 2>&1",
                 wspr_c.rcall, wspr_c.rgrid, w->arun_cf_MHz, year % 100, month,
                 day, dp->hour, dp->min, dp->snr, dp->dt_print, (int)dp->drift1,
                 dp->freq_print, dp->call, dp->grid, dp->pwr);
        ...
        ext_send_msg_encoded(w->rx_chan, WSPR_DEBUG_MSG, "EXT", "WSPR_UPLOAD",
                             "%02d%02d %3.0f %4.1f %9.6f %2d %s", dp->hour,
                             dp->min, dp->snr, dp->dt_print, dp->freq_print,
                             (int)dp->drift1, dp->c_l_p);
      }
      wspr_printf(
          "UPLOAD U%d/%d %s"
          "%02d%02d %3.0f %4.1f %9.6f %2d %s"
          "\n",
          i, w->uniques, w->autorun ? "autorun " : "", dp->hour, dp->min,
          dp->snr, dp->dt_print, dp->freq_print, (int)dp->drift1, dp->c_l_p);
      TaskSleepMsec(1000);
    }
    ...
    if (w->autorun) {
      ...
      if (now - w->arun_last_status_sent > MINUTES_TO_SEC(6)) {
        ...
        asprintf(&w->arun_stat_cmd,
                 "curl "
                 "'http://wsprnet.org/"
                 "post?function=wsprstat&rcall=%s&rgrid=%s&rqrg=%.6f&tpct=0&"
                 "tqrg=%.6f&dbm=0&version=1.3+Kiwi' >/dev/null 2>&1",
                 wspr_c.rcall, wspr_c.rgrid, w->arun_cf_MHz, w->arun_cf_MHz);
        // printf("AUTORUN %s\n", w->arun_stat_cmd);
        non_blocking_cmd_system_child("kiwi.wsprnet.org", w->arun_stat_cmd,
                                      NO_WAIT);
      }
      if (w->arun_decoded > w->arun_last_decoded) {
        input_msg_internal(w->arun_csnd, (char *)"SET geo=%d%%20decoded",
                           w->arun_decoded);
...
void wspr_data(int rx_chan, int ch, int nsamps, TYPECPX *samps) {
  ...
  if (sec != w->last_sec) {
    if (min & 1 && sec == 40) w->abort_decode = true;
  ...
  if (w->tsync == FALSE) {  // sync to even minute boundary
    if (!(min & 1) && (sec == 0)) {
      ...
      w->fi = 0;
      if (w->status != DECODING) wspr_status(w, RUNNING, RUNNING);
      w->tsync = TRUE;
    }
  }
  ...
  if (w->didx == 0) {
    ...
    memset(&w->buf->pwr_sampavg[w->ping_pong][0], 0,
           sizeof(w->buf->pwr_sampavg[0]));
  }

  if (w->group == 0) w->utc[w->ping_pong] = utc_time();

  wspr_array_dim(w->ping_pong, N_PING_PONG);
  WSPR_CPX_t *idat = w->buf->i_data[w->ping_pong],
             *qdat = w->buf->q_data[w->ping_pong];
  // double scale = 1000.0;
  double scale = 1.0;
  ...
  // interpolation, decimation and FIR filtering (like the new audio
  // re-sampler).
  //
  // Maybe we were thinking of the old "drop-sampling" *interpolator* used by
  // audio. Although even that relies on the passband FIR doing the necessary
  // pre-interpolation filtering. And a post-interpolation LPF is also required.
  //
  // Much more efficient to raise the audio bandwith to 12 kHz and be able to
  // use *integer* decimation (12k / 32 = 375 Hz) here like we originally did
  // when the WSPR extension was first written (before the audio b/w was changed
  // to accomodate the S4285 extension).
...
#define FRACTIONAL_DECIMATION 0
  ...
  for (i = trunc(w->fi); i < nsamps;) {
    ...
    if ((w->didx % NFFT) == (NFFT - 1)) {
      // wspr_printf("SAMPLER pp=%d grp=%d frate=%.1f fdecimate=%.1f
      // rx_chan=%d\n",
  ...
  for (i = 0; i < nsamps; i++) {
    // decimate
    if (w->decim++ < (int_decimate - 1)) continue;
    w->decim = 0;

    if (w->didx >= TPOINTS) return;
    ...
    if ((w->didx % NFFT) == (NFFT - 1)) {
      ...
      if (w->group) {  // skip first to pipeline
        w->fft_wakeups++;
        TaskWakeup(w->WSPR_FFTtask_id, TWF_CHECK_WAKING,
                   TO_VOID_PARAM(w->rx_chan));
      }
      w->group++;
    }
...
bool wspr_msgs(char *msg, int rx_chan) {
  ...
  if (n == 1) {
    if (w->capture) {
      if (!w->create_tasks) {
        w->WSPR_FFTtask_id =
            CreateTaskF(WSPR_FFT, TO_VOID_PARAM(rx_chan), EXT_PRIORITY,
                        CTF_RX_CHANNEL | (rx_chan & CTF_CHANNEL));
        w->WSPR_DecodeTask_id =
            CreateTaskF(WSPR_Deco, TO_VOID_PARAM(rx_chan), EXT_PRIORITY_LOW,
                        CTF_RX_CHANNEL | (rx_chan & CTF_CHANNEL));
        w->create_tasks = true;
      }
...
static double wspr_cfs[] = {137.5,   475.7,    1838.1,   3570.1,   3594.1,
                            5288.7,  5366.2,   7040.1,   10140.2,  14097.1,
                            18106.1, 21096.1,  24926.1,  28126.1,  50294.5,
                            70092.5, 144490.5, 432301.5, 1296501.5};

static struct mg_connection wspr_snd_mc[MAX_RX_CHANS],
    wspr_ext_mc[MAX_RX_CHANS];
...
#define WSPR_FILTER_BW 300
  ...
  clprintf(csnd,
           "WSPR autorun: which=%d idx=%d off=%.2f if=%.2f df=%.2f cf=%.2f "
           "cfo=%.0f\n",
           which, idx, freq_offset, if_freq_kHz, dial_freq_kHz, center_freq_kHz,
           cfo);

  double max_freq = freq_offset + ui_srate / 1e3;
  if (dial_freq_kHz < freq_offset || dial_freq_kHz > max_freq) {
    clprintf(csnd,
             "WSPR autorun: ERROR dial_freq_kHz %.2f is outside rx range %.2f "
             "- %.2f\n",
  ...
  input_msg_internal(
      csnd,
      (char *)"SET agc=1 hang=0 thresh=-100 slope=6 decay=1000 manGain=50");
  input_msg_internal(csnd,
                     (char *)"SET mod=usb low_cut=%d high_cut=%d freq=%.2f",
                     WSPR_BFO - WSPR_FILTER_BW / 2,
                     WSPR_BFO + WSPR_FILTER_BW / 2, if_freq_kHz);
  ...
  input_msg_internal(
      cext, (char *)"SET ext_switch_to_client=wspr first_time=1 rx_chan=%d",
      chan);
  ...
  input_msg_internal(cext, (char *)"SET dialfreq=%.2f cf_offset=%.0f",
                     dial_freq_kHz, cfo);
  ...
  asprintf(&w->arun_stat_cmd,
           "curl "
           "'http://wsprnet.org/"
           "post?function=wsprstat&rcall=%s&rgrid=%s&rqrg=%.6f&tpct=0&tqrg=%."
           "6f&dbm=0&version=1.3+Kiwi' >/dev/null 2>&1",
           wspr_c.rcall, wspr_c.rgrid, w->arun_cf_MHz, w->arun_cf_MHz);
...
ext_t wspr_ext = {"wspr",    wspr_main,       wspr_close,
                  wspr_msgs, EXT_NEW_VERSION, EXT_FLAGS_HEAVY};
...
void wspr_main() {
  ...
  // NB: must be called before shmem_ipc_setup() since it initializes
  // referenced, but non-shared, memory
  wspr_init();
  ...
  for (i = 0; i < rx_chans; i++) {
    ...
    w->fftin =
        (WSPR_FFTW_COMPLEX *)WSPR_FFTW_MALLOC(sizeof(WSPR_FFTW_COMPLEX) * NFFT);
    w->fftout =
        (WSPR_FFTW_COMPLEX *)WSPR_FFTW_MALLOC(sizeof(WSPR_FFTW_COMPLEX) * NFFT);
    w->fftplan = WSPR_FFTW_PLAN_DFT_1D(NFFT, w->fftin, w->fftout, FFTW_FORWARD,
                                       FFTW_ESTIMATE);
...
#ifdef WSPR_SHMEM_DISABLE
#else
    // Needs to be done as separate Linux process per channel because
    // wspr_decode() is long-running and there would be no time-slicing
    // otherwise, i.e. no NextTask() equivalent.
    shmem_ipc_setup(stprintf("kiwi.wspr-%02d", i), SIG_IPC_WSPR + i,
                    wspr_decode);
#endif
  }
  ...
  wspr_gprintf(
      "WSPR %s decimation: srate=%.6f/%d decim=%.6f/%d sps=%d NFFT=%d "
      "nbins_411=%d\n",
      FRACTIONAL_DECIMATION ? "fractional" : "integer", frate, snd_rate,
      fdecimate, int_decimate, SPS, NFFT, nbins_411);

  bool autorun =
      (*wspr_c.rcall == '\0' || wspr_c.rgrid[0] == '\0') ? false : true;
  // printf("autorun %d rcall <%s> rgrid <%s>\n", autorun, wspr_c.rcall,
  // wspr_c.rgrid);
  ...
  for (int i = 0; i < rx_chans; i++) {
    ...
    if (!err && idx != 0) {
      if (!autorun) {
        printf(
            "WSPR autorun: reporter callsign and grid square fields must be "
            "entered on WSPR section of admin page\n");
        break;
      }
      wspr_autorun(i, idx);
    }
