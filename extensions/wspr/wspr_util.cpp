...
void unpack50(u1_t *d, u4_t *call_28b, u4_t *grid_pwr_22b, u4_t *grid_15b,
              u4_t *pwr_7b) {
  *call_28b = (d[0] << 20) | (d[1] << 12) | (d[2] << 4) | (d[3] >> 4);
  *grid_pwr_22b =
      ((d[3] & 0xf) << 18) | (d[4] << 10) | (d[5] << 2) | (d[6] >> 6);
...
static const char *c =
    "0123456789ABCDEFGHIJKLMNOPQRSTUVWXYZ ";  // 37 characters
...
int unpk_(u1_t *decdata, char *call_loc_pow, char *callsign, char *grid,
  ...
  /*
   Based on the value of ntype, decide whether this is a Type 1, 2, or 3
   message.
  ...
  if ((ntype >= 0) && (ntype <= 62)) {
    ...
    *dBm = ndbm = -(ntype + 1);
    ...
    int nu = ndbm % 10;
    if ((nu != 0 && nu != 3 && nu != 7 && nu != 10) || !isalpha(grid[0]) ||
        !isalpha(grid[1]) || !isdigit(grid[2]) || !isdigit(grid[3])) {
...
int snr_comp(const void *elem1, const void *elem2) {
  const wspr_pk_t *e1 = (const wspr_pk_t *)elem1,
                  *e2 = (const wspr_pk_t *)elem2;
  int r =
      (e1->snr0 < e2->snr0)
          ? 1
          : ((e1->snr0 > e2->snr0)
                 ? -1
                 : 0);  // NB: comparison reversed to sort in descending order
  return r;
}
...
void set_reporter_grid(char *grid) {
  ...
  if (r_loc.lat != 999.0) latLon_deg_to_rad(r_loc);
}

double grid_to_distance_km(char *grid) {
  if (r_loc.lat == 999.0 || *grid == '\0') return 0;
